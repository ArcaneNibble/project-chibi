...
def main():
    ...
    # print(m_route_element_list)
    assert len(m_route_element_list) == len(m_node_data)
    # print(m_route_element_templates)
    ...
    for i in range(len(route_elems)):
        ...
        # print("    X{} Y{} Z{} I{}".format(e.x, e.y, e.z, e.index))

        # if e.m_element_enum in elem_enum_types:
        #     print("    Type: {}".format(elem_enum_types[e.m_element_enum]))
        # else:
        #     print("    Type: 0x{:04X}".format(e.m_element_enum))
        #     assert False
    ...
    # # print(m_offsets)
    # seen_fanins = {}
    # for i in range(len(m_node_data)):
    #     print("Element {} fanins:".format(i))
    #     m_fanin_edge_base, m_num_fanin_edges, m_starting_offset_idx = m_node_data[i]
    #     # print(m_fanin_edge_base, m_num_fanin_edges, m_starting_offset_idx)
    #     for j in range(m_num_fanin_edges):
    #         offset = m_offsets[m_starting_offset_idx + j]
    #         fanin_id = m_fanin_edge_base + offset
    #         fanin_id = fanin_id & 0xFFFFFFFF
    #         print("    {:08X}".format(fanin_id))
    #         if fanin_id not in seen_fanins:
    #             seen_fanins[fanin_id] = []
    #         seen_fanins[fanin_id].append(i)

    # # print(seen_fanins)
    # print("*****")
    # for edge_id in sorted(seen_fanins.keys()):
    #     print("{:08X}".format(edge_id))
    #     for elem_idx in seen_fanins[edge_id]:
    #         print("    {}".format(elem_idx))
