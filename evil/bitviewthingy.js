...
function draw_bitstream(ctx, bitstream) {
    for (let x = 0; x < WIDTH; x++) {
        for (let y = 0; y < HEIGHT; y++) {
            let [bitstream_byte, bitstream_bit] = my_coords_to_byte_bit(x, y);
    ...
    for (let x = 2; x < 8; x++) {
        for (let y = 1; y < 5; y++) {
            for (let n = 0; n < 10; n++) {
                ...
                if (!DID_ATTRIBS)
                    setattrib((x - 1) * 28, boxY, 4, 4, "LUT X" + x + "Y" + y + "N" + realN);

                // LUT inputs
                ctx.strokeStyle = LUT_INPUT_COLOR;
                outline_box(ctx, (x - 1) * 28 - 9, boxY, 9, 4);
                if (!DID_ATTRIBS)
                    setattrib((x - 1) * 28 - 9, boxY, 9, 4, "LUT X" + x + "Y" + y + "N" + realN + " inputs");
            }
            ...
            for (let nn = 0; nn < 8; nn++) {
                ...
                if (!DID_ATTRIBS)
                    setattrib(
                        ...
                        LUTYLOCS[4 - y] + nn * 2,
                        4, 2,
                        "LAB X" + x + "Y" + y + " LOCAL_INTERCONNECT " + (nn + 5));
                outline_box(ctx,
                    (x - 1) * 28 - 13,
                ...
                if (!DID_ATTRIBS)
                    setattrib(
                        ...
                        LUTYLOCS[4 - y] + 30 + nn * 2,
                        4, 2,
                        "LAB X" + x + "Y" + y + " LOCAL_INTERCONNECT " + (25 - nn));
            }
            ...
            for (let nn = 0; nn < 5; nn++) {
                ...
                if (!DID_ATTRIBS)
                    setattrib(
                        ...
                        LUTYLOCS[4 - y] + nn * 4 + 2,
                        4, 2,
                        "LAB X" + x + "Y" + y + " LOCAL_INTERCONNECT " + nn);
                outline_box(ctx,
                    (x - 1) * 28 + 7,
                ...
                if (!DID_ATTRIBS)
                    setattrib(
                        ...
                        LUTYLOCS[4 - y] + nn * 4 + 26,
                        4, 2,
                        "LAB X" + x + "Y" + y + " LOCAL_INTERCONNECT " + (17 - nn));
            }
        }
    }
            ...
            for (let nn = 0; nn < 4; nn++) {
                ...
                if (!DID_ATTRIBS)
                    setattrib(
                        ...
                        LUTYLOCS[4 - y] + nn * 4 + 4,
                        4, 2,
                        "Left line from X" + x + "Y" + y + " number " + nn)
                outline_box(ctx,
                    (x - 1) * 28 - 21,
                ...
                if (!DID_ATTRIBS)
                    setattrib(
                        ...
                        LUTYLOCS[4 - y] + nn * 4 + 28,
                        4, 2,
                        "Left line from X" + x + "Y" + y + " number " + (nn + 4))
            }
        }
    }
        ...
        for (let y = 1; y < 5; y++) {
            ...
            if (!DID_ATTRIBS) {
                setattrib(
                    ...
                    LUTYLOCS[4 - y] + 0,
                    4, 2,
                    "Right line from X" + x + "Y" + y + " number 0");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 6,
                    4, 2,
                    "Right line from X" + x + "Y" + y + " number 1");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 14,
                    4, 2,
                    "Right line from X" + x + "Y" + y + " number 2");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 18,
                    4, 2,
                    "Right line from X" + x + "Y" + y + " number 3");

                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 26,
                    4, 2,
                    "Right line from X" + x + "Y" + y + " number 4");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 30,
                    4, 2,
                    "Right line from X" + x + "Y" + y + " number 5");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 38,
                    4, 2,
                    "Right line from X" + x + "Y" + y + " number 6");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 44,
                    4, 2,
                    "Right line from X" + x + "Y" + y + " number 7");
            }
        }
    }
        ...
        for (let y = 1; y < 5; y++) {
            ...
            if (!DID_ATTRIBS) {
                setattrib(
                    ...
                    LUTYLOCS[4 - y] + 0,
                    4, 2,
                    "Up line from X" + x + "Y" + y + " number 0");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 4,
                    4, 2,
                    "Up line from X" + x + "Y" + y + " number 1");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 10,
                    4, 2,
                    "Up line from X" + x + "Y" + y + " number 2");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 16,
                    4, 2,
                    "Up line from X" + x + "Y" + y + " number 3");

                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 32,
                    4, 2,
                    "Up line from X" + x + "Y" + y + " number 4");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 36,
                    4, 2,
                    "Up line from X" + x + "Y" + y + " number 5");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 42,
                    4, 2,
                    "Up line from X" + x + "Y" + y + " number 6");
            }
            ...
            if (!DID_ATTRIBS) {
                setattrib(
                    ...
                    LUTYLOCS[4 - y] + 2,
                    4, 2,
                    "Down line from X" + x + "Y" + y + " number 0");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 8,
                    4, 2,
                    "Down line from X" + x + "Y" + y + " number 1");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 12,
                    4, 2,
                    "Down line from X" + x + "Y" + y + " number 2");

                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 28,
                    4, 2,
                    "Down line from X" + x + "Y" + y + " number 3");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 34,
                    4, 2,
                    "Down line from X" + x + "Y" + y + " number 4");
                setattrib(
                    (x - 1) * 28 - 17,
                    LUTYLOCS[4 - y] + 40,
                    4, 2,
                    "Down line from X" + x + "Y" + y + " number 5");
                setattrib(
                    (x - 1) * 28 - 21,
                    LUTYLOCS[4 - y] + 44,
                    4, 2,
                    "Down line from X" + x + "Y" + y + " number 6");
            }
        }
    }
...
window.onload = () => {
    ...
    canvaselem.onmousemove = e => {
        ...
        if (lastX != x || lastY != y) {
            ...
            let status =
                "(" + x + ", " + y + ")<br>" +
                "Altera bit number " + alteraLoc + "<br>" +
                "Byte 0x" + u32str(bitstream_byte) + " bit " + bitstream_bit + "<br>";
